...
dense_ = Dense(1000,init='normal', activation='relu')(input)#,,kernel_constraint=UnitNorm(axis=0)
bn_=BatchNormalization()(dense_)
dropout_=Dropout(0.5)(bn_)
dense_0 = Dense(500,init='normal', activation='relu')(dropout_)#,,kernel_constraint=UnitNorm(axis=0)
bn_1=BatchNormalization()(dense_0)
dropout_0=Dropout(0.5)(bn_1)

dense_1 = Dense(200, init='normal',activation='relu')(dropout_0)

bn_2=BatchNormalization()(dense_1)
dropout_1=Dropout(0.5)(bn_2)
dense_2 = Dense(100,init='normal', activation='relu')(dropout_1)

bn_3=BatchNormalization()(dense_2)
dropout_2=Dropout(0.5)(bn_3)
dense_4 = Dense(30,init='normal', activation='relu')(dropout_2)

bn_4=BatchNormalization()(dense_4)
dropout_3=Dropout(0.5)(bn_4)
dense_3 = Dense(10,init='normal', activation='relu')(dropout_3)  # ,

final_1 = Dense(1, activation='sigmoid')(dense_3)
...
for train_index, test_index in index:
        ...
        t = model.fit(X_train, y_train, batch_size=128, epochs=50, verbose=1,
                      callbacks=[learning_rate_reduction], validation_data=(X_cv,y_cv))  # batch_size=128ds
        print("\nTesting --------------------")
        cv_pred=np.concatenate((model.predict(X_cv),y_cv),axis=1)
...
cv_df.to_csv('DENSE6cv_results.csv')

train_pred=np.concatenate((model.predict(np.array(x_train_all.loc[:,:])),np.array(y_train_all.loc[:,:])),axis=1)
train_df=pd.DataFrame(data=train_pred, columns=['dignosis','total_status'],index=x_train_all.index)
train_df.to_csv('DENSE6train_results.csv')

test_pred=np.concatenate((model.predict(np.array(x_test_all.loc[:,:])),np.array(y_test_all.loc[:,:])),axis=1)
test_df=pd.DataFrame(data=test_pred, columns=['dignosis','total_status'],index=x_test_all.index)
test_df.to_csv('DENSE6test_results.csv')
