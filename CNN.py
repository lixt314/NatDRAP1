...
cnn=Convolution1D(nb_filter=128, filter_length=1,activation='relu',input_shape=(2048, 1))(input)
flp=Flatten()(cnn)
drop=Dropout(0.5)(flp)
final_1 = Dense(1, activation='sigmoid')(drop)
...
for train_index, test_index in index:
        X_train = np.expand_dims( np.array(x_train_all.iloc[train_index,:]),axis=2)
        X_cv = np.expand_dims( np.array(x_train_all.iloc[test_index,:]),axis=2)
        ...
        t = model.fit(X_train, y_train, batch_size=128, epochs=50, verbose=1,
                      validation_data=(X_cv,y_cv))  # batch_size=128ds
        print("\nTesting --------------------")
...
cv_df.to_csv('cnncv_results.csv')

train_pred=np.concatenate((model.predict(np.expand_dims(np.array(x_train_all.loc[:,:]),axis=2)),np.array(y_train_all.loc[:,:])),axis=1)
train_df=pd.DataFrame(data=train_pred, columns=['dignosis','total_status'],index=x_train_all.index)
train_df.to_csv('cnntrain_results.csv')

test_pred=np.concatenate((model.predict(np.expand_dims(np.array(x_test_all.loc[:,:]),axis=2)),np.array(y_test_all.loc[:,:])),axis=1)
test_df=pd.DataFrame(data=test_pred, columns=['dignosis','total_status'],index=x_test_all.index)
test_df.to_csv('cnntest_results.csv')