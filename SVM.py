...
for train_index, test_index in index:
        ...
        score_linear = clf_linear.score(X_cv, y_cv)
        print("The score of linear is : %f" % score_linear)
        print("\nTesting --------------------")
        cv_pred=np.concatenate((np.array(clf_linear.predict(X_cv))[:,np.newaxis],y_cv),axis=1)
...
cv_df.to_csv('SVMcv_results.csv')

train_pred=np.concatenate((np.array(clf_linear.predict(np.array(x_train_all.loc[:,:])))[:,np.newaxis],np.array(y_train_all.loc[:,:])),axis=1)
train_df=pd.DataFrame(data=train_pred, columns=['dignosis','total_status'],index=x_train_all.index)
train_df.to_csv('SVMtrain_results.csv')

test_pred=np.concatenate((np.array(clf_linear.predict(np.array(x_test_all.loc[:,:])))[:,np.newaxis],np.array(y_test_all.loc[:,:])),axis=1)
test_df=pd.DataFrame(data=test_pred, columns=['dignosis','total_status'],index=x_test_all.index)
test_df.to_csv('SVMtest_results.csv')
